...
| [`src/cli.ts`](src/cli.ts)                                                            | Reproducible Knip scans and normalized reports                                                   |
| [`src/generateActionableBatch.ts`](src/generateActionableBatch.ts)                    | Evidence intersection, diagnostic separation, and run artifacts                                  |
| [`src/batchLedger.ts`](src/batchLedger.ts)                                            | Batch rotation and durable attempt state                                                         |
| [`src/devinApi.ts`](src/devinApi.ts)                                                  | Devin API session creation, polling, prompt construction, and status artifacts                   |
| [`src/dispatchWorkflow.ts`](src/dispatchWorkflow.ts)                                  | Authenticated GitHub workflow dispatch used by the live Docker entrypoint                        |
| [`src/observabilityBackfill.ts`](src/observabilityBackfill.ts)                        | Historical consistent-ruler backfill                                                             |
| [`src/observabilityDashboard.ts`](src/observabilityDashboard.ts)                      | Static dashboard generation                                                                      |
| [`Dockerfile`](Dockerfile) and [`compose.yml`](compose.yml)                           | Reproducible live dispatcher and containerized test runner                                       |
| [`reports/`](reports/)                                                                | Example scan, batch, status, and dashboard artifacts                                             |
